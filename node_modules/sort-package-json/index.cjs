...
function makeIndentsMap(string, ignoreSingleSpaces) {
  ...
  for (const line of string.split(/\n/g)) {
    ...
    if (matches === null) {
      ...
      indents.set(
        key,
        entry === void 0 ? [1, 0] : [entry[0] + use, entry[1] + weight],
      )
    }
  }
  return indents
...
var sortObjectBySemver = sortObjectBy((a, b) => {
  ...
  return (0, import_compare.default)(
    (0, import_min_version.default)(aRange),
    (0, import_min_version.default)(bRange),
  )
})
...
var hasYarnOrPnpmFiles = () => {
  ...
  if (!cache.has(cwd)) {
    cache.set(
      cwd,
      import_node_fs.default.existsSync('yarn.lock') ||
        import_node_fs.default.existsSync('.yarn/') ||
        import_node_fs.default.existsSync('.yarnrc.yml') ||
        import_node_fs.default.existsSync('pnpm-lock.yaml') ||
        import_node_fs.default.existsSync('pnpm-workspace.yaml'),
    )
  }
...
var sortDependencies = onObject((dependencies, packageJson) => {
  ...
  if (shouldSortDependenciesLikeNpm(packageJson)) {
    return sortObjectByKeyNameList(dependencies, (a, b) =>
...
var sortPrettierConfig = pipe([
  ...
  onObject((config) =>
    sortObjectByKeyNameList(config, [
      ...Object.keys(config)
...
var isSequentialScript = (command) =>
...
var overFields = pipe(
  fields
    .map(({ key, over }) => (over ? overProperty(key, over) : void 0))
    .filter(Boolean),
)

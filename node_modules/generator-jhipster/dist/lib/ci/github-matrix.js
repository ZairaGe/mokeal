...
export const getUnknownGitHubMatrixProperties = (matrix) => Object.keys(matrix).filter(key => !knownGitHubMatrixProperties.has(key));
...
const randomEnvironmentMatrix = (key, options) => {
    const { useVersionPlaceholders, javaVersions = [RECOMMENDED_JAVA_VERSION, ...JAVA_COMPATIBLE_VERSIONS], nodeVersions = [RECOMMENDED_NODE_VERSION, '22', '24'], } = options;

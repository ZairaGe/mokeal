...
import { editPropertiesFileCallback } from "../base-core/support/index.js";
        ...
        return this.asPreparingTaskGroup({
            ...
            sonarSourceApi({ source }) {
                source.ignoreSonarRule = ({ ruleId, ruleKey, resourceKey, comment }) => {
                    this.editFile('sonar-project.properties', editPropertiesFileCallback([
                        {
                ...
                source.addSonarProperties = properties => {
                    ...
                    this.editFile('sonar-project.properties', editPropertiesFileCallback((lines, newValueCallback) => {
                        ...
                        for (const { key, value, valueSep, comment } of properties) {
                            const existingLine = lines.find(line => Array.isArray(line) && line[0] === key);
                            if (existingLine) {
                                existingLine[1] = newValueCallback(value, existingLine[1], valueSep);
                            }
                            else {
                                const itemsToAdd = [[key, newValueCallback(value)]];
                                if (comment) {

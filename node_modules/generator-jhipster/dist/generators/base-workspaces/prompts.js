 ...
 */
export const askForDirectoryPath = asPromptingTask(async function askForDirectoryPath({ control }) {
    ...
    await this.prompt([
        ...
        },
        {
            ...
            when: async (answers) => {
                ...
                appsFolders = (await findApplicationFolders(this, directoryPath)).filter(app => app !== 'jhipster-registry' && app !== 'registry');
...
async function findApplicationFolders(generator, directoryPath) {
    return (await readdir(generator.destinationPath(directoryPath), { withFileTypes: true }))
        .filter(dirent => dirent.isDirectory())
        .map(dirent => dirent.name)
        .filter(folder => existsSync(generator.destinationPath(directoryPath, folder, 'package.json')) &&

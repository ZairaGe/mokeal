...
import { DYNAMIC, GENERATE_SNAPSHOTS, LOCAL_BLUEPRINT_OPTION, PRIORITIES, WRITTEN, allGeneratorsConfig, defaultConfig, defaultSubGeneratorConfig, prompts, requiredConfig, subGeneratorPrompts, } from "./constants.js";
import { files, generatorFiles } from "./files.js";
import { lookupGeneratorsNamespaces } from "./internal/lookup-namespaces.js";
    ...
    get postWriting() {
        return this.asPostWritingTaskGroup({
            upgrade({ application, control }) {
                if (!application.generators)
                    return;
                if (!control.isJhipsterVersionLessThan('8.7.2'))
                    return;
                for (const generator of Object.keys(application.generators)) {
                    const commandFile = `${application.blueprintsPath}${generator}/command.${application.blueprintMjsExtension}`;
                    this.editFile(commandFile, { ignoreNonExisting: true }, content => content
                        .replace(`/**
...
`)
                    ...
                    this.editFile(generatorSpec, { ignoreNonExisting: true }, content => content.replaceAll(/blueprint: '([\w-]*)'/g, "blueprint: ['$1']"));
                }
            },
            ...
            addCliToPackageJson({ application }) {
                ...
                if (!cli || !cliName || this.jhipsterConfig[LOCAL_BLUEPRINT_OPTION])
                    return;
                this.packageJson.merge({
                    bin: {
                        [cliName]: 'cli/cli.cjs',
    ...
    get postInstall() {
        return this.asPostInstallTaskGroup({
            async addSnapshot({ control }) {
                ...
                const { skipInstall, skipGit, [GENERATE_SNAPSHOTS]: generateSnapshots = !localBlueprint && !skipInstall && !skipGit && !control.existingProject, } = this.options;

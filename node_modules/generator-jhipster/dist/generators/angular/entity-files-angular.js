...
export const postWriteEntitiesFiles = asPostWritingEntitiesTask(async function (taskParam) {
    ...
    const entities = (application.filterEntitiesForClient ?? filterEntitiesForClient)(taskParam.entities).filter(entity => !entity.builtInUser && !entity.embedded && !entity.entityClientModelOnly);
    if (application.generateUserManagement && application.userManagement.skipClient) {
...
export const cleanupEntitiesFiles = asWritingEntitiesTask(function ({ application, entities, control, }) {
    for (const entity of (application.filterEntitiesForClient ?? filterEntitiesForClient)(entities).filter(entity => !entity.builtIn)) {
        ...
        if (control.isJhipsterVersionLessThan('8.12.0')) {
            ...
            this.removeFile(`${application.clientSrcDir}app/entities/${entityFolderName}/delete/${entityFileName}-delete-dialog.component.spec.ts`);

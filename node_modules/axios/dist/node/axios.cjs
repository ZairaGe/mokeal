...
const own = (obj, key) =>
  obj != null && utils$1.hasOwnProp(obj, key) ? obj[key] : undefined;
...
function setProxy(
  ...
  if (proxy) {
    ...
    if (proxyAuth) {
      ...
      const authUsername =
        authIsObject && utils$1.hasOwnProp(proxyAuth, "username")
          ? proxyAuth.username
          : undefined;
      const authPassword =
        authIsObject && utils$1.hasOwnProp(proxyAuth, "password")
          ? proxyAuth.password
          : undefined;
  ...
  function httpAdapter(config) {
    return wrapAsync(
      async function dispatchHttpRequest(resolve, reject, onDone) {
        const own = (key) =>
          utils$1.hasOwnProp(config, key) ? config[key] : undefined;
        let data = own("data");
        ...
        if (config.socketPath) {
          ...
          if (config.allowedSocketPaths != null) {
            ...
            const isAllowed = allowed.some(
              (entry) =>
                typeof entry === "string" &&
                path.resolve(entry) === resolvedSocket,
            );
            if (!isAllowed) {
              return reject(
                new AxiosError(
        ...
        req = transport.request(options, function handleResponse(res) {
          ...
          const streams = [res];
          const responseLength = utils$1.toFiniteNumber(
...
var resolveConfig = (config) => {
  ...
  const own = (key) =>
    utils$1.hasOwnProp(newConfig, key) ? newConfig[key] : undefined;
  const data = own("data");
...
const factory = (env) => {
  ...
  return async (config) => {
    ...
    const hasMaxContentLength =
      utils$1.isNumber(maxContentLength) && maxContentLength > -1;

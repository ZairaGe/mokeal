...
import { createDayjsUpdateLanguagesEditFileCallback, createWebpackUpdateLanguagesNeedleCallback, } from "../../../client/support/update-languages.js";
import { generateLanguagesWebappOptions } from "../../../languages/support/languages.js";
import { AngularApplicationGenerator } from "../../generator.js";
        ...
        return this.asPreparingTaskGroup({
            translations({ application }) {
                application.addLanguageCallbacks.push((_newLanguages, allLanguages) => {
                    ...
                    this.editFile(`${clientSrcDir}app/shared/language/find-language-from-key.pipe.ts`, { ignoreNonExisting }, createNeedleCallback({
                        contentToAdd: generateLanguagesWebappOptions(allLanguages),
                        needle: 'jhipster-needle-i18n-language-key-pipe',
                    }));
                    this.editFile(`${clientSrcDir}app/config/language.constants.ts`, { ignoreNonExisting }, createNeedleCallback({
                    ...
                    if (application.clientBundlerWebpack) {
                        this.editFile(`${clientRootDir}webpack/webpack.custom.js`, { ignoreNonExisting }, createWebpackUpdateLanguagesNeedleCallback(allLanguages, this.relativeDir(clientRootDir, clientI18nDir)));
                    }
                    else if (application.clientBundlerEsbuild) {
                        this.editFile(`${application.clientI18nDir}index.ts`, createNeedleCallback({
                            needle: 'i18n-language-loader',
                            contentToAdd: allLanguages.map(lang => `'${lang.languageTag}': async (): Promise<any> => import('i18n/${lang.languageTag}.json').catch(),`),
                        }), createNeedleCallback({
                            needle: 'i18n-language-angular-loader',
                            contentToAdd: allLanguages
                                .filter(lang => lang.angularLocale)
                                .map(lang => `'${lang.languageTag}': async (): Promise<void> => import('@angular/common/locales/${lang.angularLocale}'),`),
                        }));
                    }
                });
            },

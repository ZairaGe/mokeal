...
import { askForAdminPassword, askForApps, askForClustersMode, askForDockerPushCommand, askForDockerRepositoryName, askForMonitoring, askForPath, askForServiceDiscovery, } from "../../../base-workspaces/internal/docker-prompts.js";
import { getJdbcUrl } from "../../../spring-boot/generators/data-relational/support/index.js";
import { BaseKubernetesGenerator } from "../../generator.js";
import { checkHelm } from "../../kubernetes-base.js";
import { askForIngressDomain, askForKubernetesNamespace } from "../../prompts.js";
import { applicationHelmFiles, applicationKnativeFiles, applicationKubernetesFiles, deploymentHelmFiles, deploymentKnativeFiles, deploymentKubernetesFiles, } from "./files.js";
        ...
        return this.asInitializingTaskGroup({
            ...
            async checkKnative() {
                if (this.skipChecks)
                    return;
                try {
                    await this.spawnCommand(String.raw `kubectl get deploy -n knative-serving --label-columns=serving.knative.dev/release | grep -E "v0\.[8-9]{1,3}\.[0-9]*`);
                }
                catch {
                ...
                if (deployment.dockerRepositoryName) {
                    ...
                    for (const app of applications) {
                        ...
                        if (app.buildTool === MAVEN) {
                            runCommand = `./mvnw -ntp -Pprod verify jib:build${process.arch === 'arm64' ? ' -Djib-maven-plugin.architecture=arm64' : ''} -Djib.to.image=${app.targetImageName}`;
                        }
                        else {
                            runCommand = `./gradlew bootJar -Pprod jibBuild${process.arch === 'arm64' ? ' -PjibArchitecture=arm64' : ''} -Djib.to.image=${app.targetImageName}`;
                        }

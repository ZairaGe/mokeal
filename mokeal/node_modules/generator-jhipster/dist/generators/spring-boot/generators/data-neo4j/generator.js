...
import { SpringBootApplicationGenerator } from "../../generator.js";
        ...
        return this.asConfiguringEachEntityTaskGroup({
            async configuringEachEntity({ entityConfig }) {
                if (entityConfig.dto && entityConfig.dto !== 'no') {
                    this.log.warn(`The DTO option is not supported for Neo4j database. Neo4j persists the entire constellation, DTO causes the constellation to be incomplete. DTO is found in entity ${entityConfig.name}.`);
                }
            },
        });
    }
            ...
            async checkUserRelationship({ entities }) {
                entities.forEach(entity => {
                    if (entity.relationships.some(relationship => relationship.otherEntity.builtInUser)) {
                        this.log.warn(`Relationship with User entity should be avoided for Neo4j database. Neo4j persists the entire constellation, related User entity will be updated causing security problems. Relationship with User entity is found in entity ${entity.name}.`);
                    }
                });
            },
        });

...
import { JavascriptSimpleApplicationGenerator } from "../../generator.js";
    ...
    get preparing() {
        return this.asPreparingTaskGroup({
            loadNodeDependencies({ application }) {
                ...
                this.loadNodeDependenciesFromPackageJson(application.nodeDependencies, this.fetchFromInstalledJHipster('javascript-simple-application', 'resources', 'package.json'));
            },
            source({ source }) {
                source.addEslintConfig = ({ import: importToAdd, config }) => this.editFile('eslint.config.ts', config ? createNeedleCallback({ needle: 'eslint-add-config', contentToAdd: config }) : content => content, importToAdd ? createNeedleCallback({ needle: 'eslint-add-import', contentToAdd: importToAdd }) : content => content);
                source.addEslintIgnore = ({ ignorePattern }) => source.addEslintConfig({ config: `{ ignores: '${ignorePattern.replaceAll("'", String.raw `\')}`)},` });
            },
        });
    }

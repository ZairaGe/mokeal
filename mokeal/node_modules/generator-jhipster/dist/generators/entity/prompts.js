...
import { APPLICATION_TYPE_GATEWAY } from "../../lib/core/application-types.js";
...
const { BIG_DECIMAL, BOOLEAN, DOUBLE, DURATION, ENUM, FLOAT, INTEGER, INSTANT, LOCAL_DATE, LONG, STRING, UUID, ZONED_DATE_TIME, LOCAL_TIME, } = CommonDBTypes;
...
export const askForMicroserviceJson = asPromptingTask(async function askForMicroserviceJson() {
...
export const askForUpdate = asPromptingTask(async function askForUpdate() {
    const context = this.entityData;
...
export function askForFieldsToRemove() {
    const context = this.entityData;
...
export const askForRelationships = function askForRelationships(...args) {
    const context = this.entityData;
    // don't prompt if data is imported from a file
...
export function askForRelationsToRemove() {
    const context = this.entityData;
    // prompt only if data is imported from a file
    ...
    return this.prompt([
        ...
        if (props.confirmRemove) {
            ...
            for (let i = relationships.length - 1; i >= 0; i -= 1) {
                ...
                if (props.relsToRemove.includes(`${rel.relationshipName}:${rel.relationshipType}`)) {
...
export function askForFiltering() {
    const context = this.entityData;
...
export function askForReadOnly() {
    const context = this.entityData;
    // don't prompt if data is imported from a file
...
export function askForPagination() {
    const context = this.entityData;

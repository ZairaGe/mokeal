...
import { buildJDLApplicationConfig } from "../lib/jdl-config/jhipster-jdl-config.js";
...
const addCommandGeneratorOptions = async (command, generatorMeta, { blueprintOptionDescription } = {}) => {
...
export const buildCommands = ({ program, commands = {}, envBuilder, env, loadCommand = async (key) => {
    ...
    for (const extension of ['js', 'mjs', 'ts']) {
        try {
            const { default: command } = await import(__rewriteRelativeImportExtension(`${module}.${extension}`));
            return command;
        }
        catch (e) {
            error = e;
